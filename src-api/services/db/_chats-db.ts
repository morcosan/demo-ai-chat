...
const renameChat = async (chat: DbChat) => {
	...
	if (!hasDevGPT) {
		...
		if (await ChatGPT4oMini.isAvailable()) response = await ChatGPT4oMini.getResponse(config, gptMessages)
		if (await Claude35Haiku.isAvailable()) response = await Claude35Haiku.getResponse(config, gptMessages)
