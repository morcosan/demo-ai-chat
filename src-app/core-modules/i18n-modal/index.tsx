...
export const I18nModal = ({ opened, onClose }: Props) => {
			...
			>
				<span className="flex w-full items-center text-left">
					{isLoading ? (
						...
						<item.flag className="mx-xs-6 w-sm-0 min-w-sm-0" style={{ fill: 'initial', stroke: 'initial' }} />
					)}
	...
	return (
		<Modal opened={opened} width="lg" title={t('core.action.changeLanguage')} noFooter onClose={onClose}>
			<div className="flex flex-col gap-sm-5">
				{regionItems.map((region) => (
					<div key={region.key}>
						<div className="mb-xs-5 text-size-lg">{region.title}</div>

						<div className="grid grid-cols-1 gap-x-xs-5 gap-y-xs-1 xs:grid-cols-2 sm:grid-cols-3 lg:grid-cols-4">
							{region.items.map(renderItem)}
						</div>
					</div>
				))}
			</div>
