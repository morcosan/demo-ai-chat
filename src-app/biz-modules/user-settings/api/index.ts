import { AccountApiPayload, AccountDTO, BillingApiPayload, BillingDTO, mainAPI, STATUS__SUCCESS } from '@app/api'
...
export const API = {
	async getAccount(): Promise<Account | null> {
		...
		return resp.status === STATUS__SUCCESS && resp.data ? mapAccount(resp.data) : null
	},

	async getBilling(): Promise<Billing | null> {
		const resp = await mainAPI.get<BillingDTO>('/api/billing', {})

		return resp.status === STATUS__SUCCESS && resp.data ? mapBilling(resp.data) : null
	},
